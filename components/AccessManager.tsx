...
const AccessManager: React.FC<AccessManagerProps> = ({ isOwner, onClose }) => {
  ...
  const handleAdd = async () => {
    ...
    const targetRef = userType === 'admin' ? dbRefs.admins() : dbRefs.students();
    
    try {
      await push(targetRef, { u: newU, p: newP });
      setNewU(''); setNewP('');
      alert(`${userType === 'admin' ? 'Admin' : 'Student'} সফলভাবে যোগ করা হয়েছে!`);
    } catch (err) {
              ...
              <div className="space-y-8">
                  ...
                  <div className="space-y-3">
                    ...
                    <button 
                      ...
                      className={`w-full py-4 font-bold rounded-xl shadow-lg transition-all text-white mt-2
                        ${userType === 'admin' ? 'bg-purple-600 hover:bg-purple-700 shadow-purple-200' : 'bg-indigo-600 hover:bg-indigo-700 shadow-indigo-200'}
                      `}
                    >
                      + Add {userType === 'admin' ? 'Admin' : 'Student'}
                    </button>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="grid gap-3">
                    {Object.keys(admins).length === 0 && <p className="text-slate-400 text-xs italic">No additional admins.</p>}
